  ...
  for (const failure of audit.expectedFailures) {
    ...
    try {
      ...
      if (result.fixed) {
        ...
        manual.push({ ...failure, ...result, fixed: undefined });
      }
    } catch (err) {
...
const FIXERS = {
   ...
   *
   * 架构定位：语义提取应由 sdd-archive skill → LLMWiki MCP 执行。
   * harness 层只做确定性预检：确认 glossary 段存在且 source 可用，
   * 然后标记为 needs-llm，由 skill 层的 LLM 完成实际提取。
   */
  async CONCEPTS_EXTRACTED({ changeDir, wikiDir: _wikiDir }) {
    const findings = await readText(path.join(changeDir, 'findings.md'));
    if (!findings) return { fixed: false, reason: 'findings.md 不存在' };
    ...
    // 不在 harness 层用正则提取——语义理解是 LLMWiki skill 的职责。
    // harness 只确认 source 可用，然后输出执行指令。
    return {
      fixed: false,
      needsLLM: true,
      reason: '概念提取需要语义理解，应由 sdd-archive skill → LLMWiki MCP 执行',
      instruction: `从 findings.md 的 glossary 段提取概念定义，写入 wiki/concepts/<slug>.md（带 type:concept frontmatter）。source: ${path.join(changeDir, 'findings.md')}`,
    };
  },

  /**
   * ENTITIES_EXTRACTED: 需要 LLM 语义理解，标记为 needs-llm
   */
  async ENTITIES_EXTRACTED({ changeDir }) {
    return {
      fixed: false,
      needsLLM: true,
      reason: 'entity 关系图谱需要语义理解，应由 sdd-archive skill → LLMWiki MCP 执行',
      instruction: `从 design.md + specs/ 中提取实体关系（模块间依赖、数据流），写入 wiki/entities/<slug>.md（带 type:entity frontmatter）。source: ${path.join(changeDir, 'design.md')}`,
    };
  },
