...
function evaluateStateTransition(transition) {
  if (transition?.assertion === 'delta') {
    ...
    // 浮点差值用 epsilon 比较，避免 0.1+0.2=0.30000000000000004 这类假败
    const FLT_EPSILON = 1e-9;
    return {
      pass: Math.abs(actual - expected) < FLT_EPSILON,
      actual,
...
async function collectSourceFiles(root) {
  ...
  async function walk(dir, depth = 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isSymbolicLink()) continue;  // 防 symlink 循环 + 逃逸到 ignoredDirs 之外
...
// 三处共用：收集 src/app/pages 下的源码 + 并行读取内容（有界并发）
async function collectSourceContent(projectDir) {
  const sourceRoots = [
  ...
  let cursor = 0;
  const workers = new Array(Math.min(READ_CONCURRENCY, files.length)).fill(0).map(async () => {
    while (cursor < files.length) {
      const i = cursor++;
      contents[i] = await fs.readFile(files[i], 'utf8').catch(() => '');
    }
  });
...
async function findCanvasResizeFeedback(projectDir) {
  const entries = await collectSourceContent(projectDir);
  const findings = [];
  for (const { file, content } of entries) {
    if (
      ...
      if (unsafeCalls.length > 0) {
        findings.push(path.relative(projectDir, file).split(path.sep).join('/'));
      }
    }
  }
...
async function findObservationAdapter(projectDir, adapter) {
  ...
  for (const { file, content } of entries) {
    if (tokens.every((token) => content.includes(token))) {
      matches.push(path.relative(projectDir, file).split(path.sep).join('/'));
    }
  }
  return matches.sort();
}
