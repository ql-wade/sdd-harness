...
const rules = [
  ...
  // ── Archive ──────────────────────────────────────────
  {
    ...
    async check({ projectDir, changeDir }) {
      ...
      // No spec deltas in this change (e.g. docs/refactor/config-only) → nothing to promote, vacuously pass
      if (changeModules.length === 0) {
        return { pass: true, detail: '无 spec delta（纯文档/重构/配置变更，无需提升）' };
      }
      return {
        pass: promoted.length >= changeModules.length,
