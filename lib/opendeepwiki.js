...
export function getOpenDeepWikiUrl() {
  return process.env.OPENDEEPWIKI_URL || `http://localhost:${process.env.OPENDEEPWIKI_PORT || 8090}`;
}
...
export function checkOpenDeepWikiHealth() {
  ...
  try {
    ...
    return { running: true, url, healthy: true };
  } catch {
...
export async function ensureComposeFile(templatesDir) {
  ...
  if (await fs.pathExists(src)) {
    await fs.copy(src, COMPOSE_FILE, { overwrite: false });
  }
}
