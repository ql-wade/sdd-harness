...
export function checkOpenDeepWikiHealth() {
  ...
  try {
   const result = execSync(`curl -sf ${url}/health 2>/dev/null`, {
    ...
    try {
      const mcpCheck = execSync(`curl -sf -o /dev/null -w "%{http_code}" ${url}/api/mcp 2>/dev/null`, {
        encoding: 'utf8',
        timeout: 5000,
      });
      // OpenDeepWiki returns 200 or 404-with-body for /api/mcp, but NOT a bare 404
      // A proxy usually returns 404 immediately
      // More reliable: check for /health returning OpenDeepWiki-style JSON
      const health = JSON.parse(result);
      isRealOpenDeepWiki = health.providers !== undefined && Array.isArray(health.providers);
      // Double-check: OpenDeepWiki should have Swagger/scalar
      try {
        const scalarCheck = execSync(`curl -sf -o /dev/null -w "%{http_code}" ${url}/v1/scalar 2>/dev/null`, {
          encoding: 'utf8',
          timeout: 5000,
        });
        if (scalarCheck === '200') isRealOpenDeepWiki = true;
      } catch {}
    } catch {}
