  ...
  if (!dryRun) {
    ...
    if (graphHealth.existence === 'missing') {
      ...
      await fs.writeFile(graphPath, JSON.stringify({
        note: 'placeholder. Real graph generated by sdd graph refresh (run /understand in Claude Code)',
  ...
  .action(async (options) => {
    ...
    if (graphHealth.existence === 'missing') {
      ...
      if (hasIssues) {
        ...
        console.log(chalk.gray('     Fix: sdd graph refresh (run /understand in Claude Code)'));
      } else {
