...
| grill-with-docs skill | pre-OpenSpec 的业务澄清 |
...
- **Wrapper skill**：`sdd-grill`（封装 grill-with-docs + MCP context pack + discovery 路由）
...
- **底层**：grill-with-docs、`sdd-context-mcp.build_grill_pack`
...
sdd init 流程：
  ...
  3. clone skills      —— Superpowers / planning-with-files / grill-with-docs → 平台 skills 目录
...
| grill-with-docs（skill） | stage·grill | 业务澄清 | clone 到 skills 目录 | 文件存在 |
...
| `sdd-grill` | 业务澄清 | grill context pack | findings.md | grill-with-docs, sdd-context-mcp |
